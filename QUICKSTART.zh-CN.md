...
Lore Forge 需要 **文件读写**、**跨会话记忆**、**内容搜索** 三项能力。

最简单的方式：让 Agent 帮你一键安装：

```text
读取 REQUIREMENT.zh-CN.md。检测你当前运行在哪个平台。
为这个平台安装最低需要的 MCP 服务器。
安装完成后验证并报告当前已具备哪些能力。
```

然后重启平台即可。

详见 [REQUIREMENT.zh-CN.md](./REQUIREMENT.zh-CN.md) 的手动安装和各平台详细说明。
...
然后它会以**延迟写入模式**开始概念发现访谈——对话快速流畅，回答先记录到临时文件，在你确认时才批量写入 Story Bible 文件。
...
自然地回答每个问题。你的回答会暂存在 scratch 文件中以保持对话流畅。当你准备好时，说 `写入` 来构建 Story Bible 文件，或说 `提交` 来构建后自动 git commit。
